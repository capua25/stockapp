...
- [x] **Hallazgo 7 — 403 "Prohibido" cuando el directorio de logs existe pero el servidor no puede leerlo.** Encontrado corriendo la app real (post Hallazgos 1-6), NO por review de código. `ServicioConsultaLogs.ListarArchivos` dejaba escapar el `UnauthorizedAccessException` crudo de `Directory.GetFiles` cuando el directorio EXISTE pero el proceso no tiene permiso de lectura; `DomainExceptionHandler` lo mapeaba al mismo 403 "Prohibido." que un usuario sin permiso — mensaje engañoso en la única pantalla que tiene el admin para diagnosticar sin acceso al servidor, porque lo manda a revisar SU cuenta en vez del directorio del servidor. Segundo filo: si el directorio existe sin permisos (a diferencia de "no existe"), tampoco había aviso por consola al arrancar — el `try/catch` de `Program.cs` solo cubría el fallo de `Directory.CreateDirectory`, que es no-op cuando el directorio ya existe (no valida que sea usable). Fix: `DirectorioLogsInaccesibleException` nueva en `StockApp.Domain.Exceptions`, lanzada desde `ServicioConsultaLogs.ListarArchivos` con la ruta en el mensaje; `DomainExceptionHandler` la mapea a 503 (no 403) con el `detail` visible — a diferencia del 500 genérico, acá el mensaje SÍ tiene que llegar al admin. `Program.cs` agrega una prueba de escritura (crear+borrar un archivo de prueba) inmediatamente después de `Directory.CreateDirectory`, dentro del mismo `try`, así el caso "directorio existe sin permisos" cae al mismo `catch` que ya avisaba por consola. La variante "padre sin permisos + subdirectorio inexistente" (que sí funcionaba bien) queda intacta: `Directory.CreateDirectory` sigue fallando ahí ANTES de llegar a la prueba nueva. Commit: `fix(diagnostico): distinguir permisos de filesystem de permisos de usuario en logs`.
