...
**`FechaVencimiento` (revisión del usuario sobre este plan)**: `GastoConfirmarDto.FechaVencimiento` (Task 2) se mapea directo a `Gasto.FechaVencimiento` en esta task, con la MISMA conversión `DateOnly?` → `DateTime?` UTC que `Fecha` (arriba) — pero solo cuando tiene valor: `gastoDto.FechaVencimiento is { } f ? new DateTime(f.Year, f.Month, f.Day, 0, 0, 0, DateTimeKind.Utc) : (DateTime?)null`. Task 3 ya garantiza que viene con valor para todo gasto `Credito` (si no, `ValidacionImportacionException` corta antes de llegar acá) — así que en la práctica esta task nunca ve un `Credito` sin vencimiento, pero el mapeo tiene que ser null-safe igual porque `Contado` sí llega con `FechaVencimiento = null` legítimamente.
...
### Gaps de datos — el test hace de humano (decisión ya tomada, no queda pendiente)

`MovimientoPoaAnalizadoDto` (F5b) — la fuente de los movimientos `CompromisoSoloPoa` que hay que convertir en `GastoConfirmarDto` nuevos — NO trae `CodigoRubro`, `Fecha` ni `FechaVencimiento` (una hoja de línea POA no tiene columnas de rubro ni de fecha por movimiento). El contrato de `GastoConfirmarDto` (spec §3 + Task 2) exige los tres como obligatorios para un gasto `Credito`. Esto es un gap real entre lo que F5a/F5b capturan y lo que F5c necesita para escribir un compromiso.

**Ya se decidió con el usuario cómo se resuelve en producción, y NO es con un rubro sentinel**: en F5d, el admin completa `CodigoRubro`, `Fecha` y `FechaVencimiento` fila por fila en la grilla de corrección, igual que ya completa `Programa` de las líneas POA (spec §2.4, decisión 4 — el mismo principio: "no se inventa dato"). Este test de aceptación corre SIN intervención humana (es CI, no un admin con una grilla), así que `ArmarPayloadConfirmacion` sintetiza esos tres valores **haciendo el papel del humano** — no es una decisión de diseño de F5c ni una alternativa a evaluar, es un valor de prueba fijo para poder ejercitar el flujo completo en un test automático:

- `CodigoRubro`: rubro sintético `999 — "Compromisos POA (sin rubro en la planilla)"`, declarado vía `MaestrosNuevos.Rubros` SOLO para este test. En producción, el admin elegiría un rubro real de los 17 existentes al completar la grilla — F5c no necesita saber cuál.
- `Fecha` y `FechaVencimiento`: `31/12/{ejercicio}`, sin impacto en ninguna de las dos aserciones del criterio de aceptación (la caja de junio se calcula solo con datos de la hoja de Gastos; el saldo POA por Literal suma `MontoTotal` sin filtrar por fecha) — un valor de prueba cualquiera sirve, el admin pondría la fecha real de la factura/orden en F5d.
