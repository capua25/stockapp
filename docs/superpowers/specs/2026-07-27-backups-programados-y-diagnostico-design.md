...
- **Entrega 1 (agregado, tercer review final E1)** — **Modo de acceso acotado** (`AccesoLimitadoViewModel`/`AccesoLimitadoView`): con la licencia vencida, `BloqueoLicenciaViewModel` es el único punto de entrada, y desde ahí el admin puede pedir un login acotado (`IngresoLimitadoSolicitado` → `ShellViewModel.MostrarLoginAccesoLimitado` → `LoginViewModel.SoloAccesoLimitado`). Un login exitoso en ese modo navega a `ShellViewModel.MostrarAccesoLimitado()`, que hostea **exclusivamente** `MantenimientoViewModel` (sin sidebar, sin `INavigationService`) — es la barrera física que impide operar el resto del sistema mientras la licencia sigue vencida. Es el camino real (y único) por el que el admin descarga el backup de seguridad cuando la API está bloqueada por licencia.
- **Entrega 1 (agregado, tercer review final E1)** — **Detalle contraintuitivo del diseño**: una fila reconciliada por `ServicioBackup.ReconciliarDumpHuerfanosAsync` (dump huérfano en disco, típicamente tras un restore) lleva `MotivoFallo` **no nulo** pese a que `Resultado == Exitosa` (ver `ServicioBackup.MarcaFilaReconciliada`). El campo se reutiliza para esa marca -en vez de agregar una columna nueva- así que ya no es cierto que `MotivoFallo` sea null siempre que `Resultado == Exitosa`. `FilaCorridaBackupVm.EsFallo`/`EsNotaInformativa` en Presentation distinguen ambos casos para no pintar esa nota informativa en rojo como si fuera un error.
