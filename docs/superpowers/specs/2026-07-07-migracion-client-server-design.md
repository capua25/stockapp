...
| 1 — Datos a PostgreSQL | **Switch TOTAL a PostgreSQL** (decidido en planificación): se remueve SQLite del código (paquetes, `UseSqlite`, las 3 migraciones viejas); un solo proveedor Npgsql y un solo set de migraciones (`InitialCreate` regenerada). La app y `Infrastructure.Tests` requieren PostgreSQL/Docker (Testcontainers) — no queda modo SQLite de respaldo ni in-memory. Transacción atómica de stock (UPDATE condicional) + tests de concurrencia. `DatabaseInitializer` reescrito (solo `MigrateAsync`) y el backup file-based (`BackupService`/`BackupPeriodicoService`, copia del `.db`) se deshabilita/remueve del arranque: `pg_dump` server-side queda diferido a la Fase 4. | Capa de datos probada sobre Postgres real; cero referencias a SQLite en el repo; el arranque ya no depende de un archivo `.db` que dejó de existir. |
...
- **Backups:** hoy `BackupService`/`BackupPeriodicoService` copian el archivo SQLite. En la Fase 1 ese mecanismo se deshabilita/remueve del arranque (con Postgres no hay `.db` que copiar); el reemplazo real (`pg_dump` programado server-side) se implementa recién en la Fase 4. Entre medio, la app corre sin backup automatizado — aceptado por ser fase de piloto/desarrollo, no de producción con datos reales.
