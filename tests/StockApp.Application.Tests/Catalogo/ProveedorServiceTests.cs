...
public class ProveedorServiceTests
{
    ...
    {
        // Guardián contra el fallo silencioso: corregir "distx" a "DistX" es un cambio
        ...
        var (svc, repo, _, _, _) = Crear();
        repo.Setup(r => r.ObtenerPorIdAsync(1)).ReturnsAsync(original);
        repo.Setup(r => r.ExisteNombreAsync("DistX", 1)).ReturnsAsync(false);

        var ex = await Record.ExceptionAsync(
            () => svc.ModificarAsync(new Proveedor { Id = 1, Nombre = "DistX", Activo = true }));

        Assert.Null(ex);
        repo.Verify(r => r.ActualizarAsync(It.Is<Proveedor>(p => p.Nombre == "DistX")), Times.Once);
    }
