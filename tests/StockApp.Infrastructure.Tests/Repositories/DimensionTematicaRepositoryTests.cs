...
public class DimensionTematicaRepositoryTests : PostgresRepositoryTestBase
{
    ...
    {
        Context.DimensionesTematicas.Add(NuevaDimension("Infraestructura"));
    ...
    {
        var id = await _repo.AgregarAsync(NuevaDimension("infraestructura"));
        Context.ChangeTracker.Clear();

        var found = await _repo.ObtenerPorIdAsync(id);
        found!.Nombre = "Infraestructura";
        await _repo.ActualizarAsync(found);
        Context.ChangeTracker.Clear();

        var updated = await _repo.ObtenerPorIdAsync(id);
        Assert.Equal("Infraestructura", updated!.Nombre);
    }
}
